...
+ (instancetype)sharedInstance {
    static LCAppContext *sharedInstance;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedInstance = [[LCAppContext alloc] init];
...
- (CGFloat)latitude {
    return [LCLocationManager sharedInstance].currentLocation.coordinate.latitude;
}

- (CGFloat)longitude {
    return [LCLocationManager sharedInstance].currentLocation.coordinate.longitude;
}

- (void)appStarted {
    self.sessionId = [[NSUUID UUID].UUIDString copy];
    [[LCLocationManager sharedInstance] startLocation];
}

- (void)appEnded {
    [[LCLocationManager sharedInstance] stopLocation];
}
...
- (BOOL)isOnline {
    BOOL isOnline = NO;
    NSString *filepath = [[NSBundle mainBundle] pathForResource:@"LCNetworkingConfiguration" ofType:@"plist"];
    if ([[NSFileManager defaultManager] fileExistsAtPath:filepath]) {
        NSDictionary *settings = [[NSDictionary alloc] initWithContentsOfFile:filepath];
        isOnline = [settings[@"isOnline"] boolValue];
    } else {
        isOnline = kLCServiceIsOnline;
    }
    return isOnline;
}
